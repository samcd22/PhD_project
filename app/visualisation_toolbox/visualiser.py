...
class Visualiser:
    ...
    def _fill_figure_metadata(self, fig, axes):
        ...
        if isinstance(self, RegressionVisualiser):
            ...
            axes[4].text(0.5,0.5, RMSE_string + ',   ' + AIC_string + ',\n' + BIC_string, fontsize = 30, va = "center", ha = 'center')
    ...
    def _show_1D_time_predictions(self, domain_points, plot_name, title, scale):
        ...
        if not os.path.exists(full_path):
            ...
            for time in np.sort(domain_points['t'].unique()):
                lower_frame = lower_pred[results_df['t'] == time]
                median_frame = median_pred[results_df['t'] == time]
                upper_frame = upper_pred[results_df['t'] == time]

                X = results_df[self.independent_variables[0]][results_df['t'] == time]
                ...
                # Lower Bound Plot
                axes[0].plot(X, lower_frame, label='Predicted ' + self.dependent_variable)
                axes[0].set_title(title_lower, fontsize=20)
                axes[0].set_xlabel(self.independent_variables[0])
                axes[0].set_ylabel(self.dependent_variable)
                axes[0].set_yscale(scale)
    ...
    def _show_2D_time_predictions(self, domain_points, plot_name, title, scale):
        ...
        if not os.path.exists(full_path):
            ...
            for time in np.sort(domain_points['t'].unique()):
                ...
                X = results_df[self.independent_variables[0]][results_df['t'] == time]
                Y = results_df[self.independent_variables[1]][results_df['t'] == time]
    ...
    def _show_3D_layers_predictions(self, domain_points, plot_name, title, scale):
        ...
        if not os.path.exists(full_path):
            ...
            # Sort points by their median prediction values
            sorted_indices = np.argsort(median_pred)
            X, Y, Z = X.iloc[sorted_indices], Y.iloc[sorted_indices], Z.iloc[sorted_indices]
            lower_pred, median_pred, upper_pred = lower_pred[sorted_indices], median_pred[sorted_indices], upper_pred[sorted_indices]
            results_df = results_df.iloc[sorted_indices]

            # Set vmin and vmax based on the entire dataset for a consistent color scale
            vmin = np.percentile([lower_pred, median_pred, upper_pred], 10)
            vmax = np.percentile([lower_pred, median_pred, upper_pred], 90)

            # Static axis limits for all frames
            min_X, max_X = np.min(X), np.max(X)
            min_Y, max_Y = np.min(Y), np.max(Y)
            min_Z, max_Z = np.min(Z), np.max(Z)

            # Define decreasing min C values for each frame
            frame_min_C_values = np.linspace(vmax, vmin, num=10)  # 10 frames, adjust as needed

            images = []
            for min_C in frame_min_C_values:
                ...
                axes[2].set_zlim(min_Z, max_Z)

                # Add Colorbar
                cbar1 = plt.colorbar(plot_1, ax=axes[3], location='top', shrink=2)
                cbar1.ax.set_title(f'Predicted {self.dependent_variable} (Min C in frame: {min_C:.2f})', fontsize=10)
    ...
    def _show_3D_time_predictions(self, domain_points, plot_name, title, scale):
        ...
        if not os.path.exists(full_path):
            ...
            for time in np.sort(domain_points['t'].unique()):
                ...
                X = results_df[self.independent_variables[0]][results_df['t'] == time]
                Y = results_df[self.independent_variables[1]][results_df['t'] == time]
                Z = results_df[self.independent_variables[2]][results_df['t'] == time]
                ...
                # Add Colorbar
                cbar1 = plt.colorbar(plot_1, ax=axes[3], location='top', shrink=2)
                cbar1.ax.set_title(f'Predicted {self.dependent_variable}', fontsize=10)
    ...
    def _animate_figures(self, images, filename):
        ...
        # Save as GIF or MP4
        imageio.mimsave(filename, frames, fps=1, loop=0)  # Use '.mp4' for video
